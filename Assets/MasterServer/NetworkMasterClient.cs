...
public class NetworkMasterClient : MonoBehaviour
{
	...
	public void InitializeClient()
	{
        ...
        client.RegisterHandler(MasterMsgTypes.GenericMessageId, OnGenericMessage);
    ...
    private void OnGenericMessage(NetworkMessage netMsg)
    {
        ...
        Debug.Log("Message received on client: " + msg.message);
    }
	...
	{
        Debug.Log("Client: requesting host list");
		if (!isConnected)
		{
	...
	void OnGUI()
	{
		...
		else
		{
            ...
            if (GUI.Button(new Rect(100, 220 + yoffset, 120, 20), "Send msg to client"))
            {
                SendMessageToClient("ladida!! (" + DateTime.Now + ")");
            }
        }
		...
		if (hosts != null)
		{
			...
			foreach (var h in hosts)
			{
				if (GUI.Button(new Rect(120, y + yoffset, 240, 20), "Host:" + h.name + "addr:" + h.hostIp + ":" + h.hostPort))
				{
                    ...
                    Debug.Log("attempting to connect.. " + h.hostIp);
                    ...
                    if (NetworkManager.singleton != null)
					{
                        ...
                        Debug.Log("Connected client! " + h.hostIp);
					}
				}
				y += 22;
			}
