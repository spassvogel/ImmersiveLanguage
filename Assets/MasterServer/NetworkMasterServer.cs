...
public class NetworkMasterServer : MonoBehaviour
{
	...
	public void InitializeServer()
	{
        ...
        NetworkServer.RegisterHandler(MasterMsgTypes.GenericMessageId, OnServerGenericMessage);

        DontDestroyOnLoad(gameObject);
	}

    private void OnServerGenericMessage(NetworkMessage netMsg)
    {
        var msg = netMsg.ReadMessage<MasterMsgTypes.GenericMessage>();
        Debug.Log("Server received message: " + msg.message);


        var response = new MasterMsgTypes.GenericMessage();
        response.message = msg.message;

        netMsg.conn.Send(MasterMsgTypes.GenericMessageId, msg);
    }
